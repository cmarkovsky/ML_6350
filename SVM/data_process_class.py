...
class DataProcessor:
    def __init__(self, path, is_bank = False, is_banknote = True):
        self.median_dict = {}
        self.features = None
        self.labels = None
        self.path = path
        if is_bank:
            ...
            self.X, self.y = self._process_banknote_data(self.path)
        else:
            self.X, self.y = self._process_concrete_data(self.path)

    def _process_banknote_data(self, path):
        col = ['variance', 'skewness', 'curtosis', 'entropy', 'y']
        self.features = col[:-1]
    ...
    def _convert_labels(self, y):
        return pd.Series(np.where(y == 1, 1, -1))
